		...
		<div id="opening" className="absolute inset-0 z-50 grid grid-cols-2 overflow-hidden">
			<div className="relative flex flex-col">
				{Array.from({ length: 21 }).map((_, index) => (
					<div key={index} className="flex-1 bg-background-dark dark:bg-background-light w-full h-full left-door" />
				))}
			...
			<div className="relative flex flex-col">
				{Array.from({ length: 21 }).map((_, index) => (
					<div key={index} className="flex-1 bg-background-dark dark:bg-background-light w-full h-full right-door" />
				))}
				<div id="logo-right" className="absolute top-0 left-0 w-full h-full flex justify-start items-center">
					<svg version="1.2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1024 1024" className="relative aspect-square -translate-x-1/2 h-80 fill-primary">
